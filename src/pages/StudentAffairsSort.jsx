// Copy of FacultySecretarySort.jsx for StudentAffairsSort.jsx
...
export default function StudentAffairsSort() {
  ...
  useEffect(() => {
    async function fetchFacultiesAndFiles() {
      try {
        const res = await fetch('http://localhost:8080/api/faculties');
        if (res.ok) {
          const faculties = await res.json();
          const filesWithStatus = await Promise.all(faculties.map(async fac => {
            try {
              const getRes = await fetch(`http://localhost:8080/api/auth/faculty-sorted-students-csv?facultyId=${fac.facultyID}`, { method: 'GET' });
              return {
          ...
          setFacultyFiles(filesWithStatus);
        }
      } catch (e) {
        setFacultyFiles([]);
      }
    }
    fetchFacultiesAndFiles();
  }, []);

  async function handleSort() {
    try {
      const res = await fetch('http://localhost:8080/api/auth/student-affairs/sort-all-students', { method: 'POST' });
      if (res.ok) {
        setSortedReady(true);
  ...
  // Button color logic
  const allFacultiesReady = facultyFiles.length > 0 && facultyFiles.every(f => f.hasFile);
  const sortButtonActive = allFacultiesReady;
  const sortedDownloadActive = allFacultiesReady && sortedReady;
  const savedInSystemGreen = sortedReady;

  // Dummy CSV content for download simulation
  const dummyDepartmentCSV = 'id,name\n1,Student A\n2,Student B';

  // Department names from the provided image
  const facultyNames = [
    'Faculty of Science',
  ...
  async function handleDownloadSortedList(facultyId, facultyName) {
    try {
      const res = await fetch(`http://localhost:8080/api/auth/faculty-sorted-students-csv?facultyId=${facultyId}`);
      if (!res.ok) throw new Error('Failed to download sorted list');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${facultyName.replace(/\s+/g, '_').toLowerCase()}_sorted_students.csv`;
      document.body.appendChild(a);
      a.click();
      a.remove();
  ...
  async function handleDownloadStudentAffairsSorted() {
    try {
      const res = await fetch('http://localhost:8080/api/auth/student-affairs/sorted-students-csv');
      if (!res.ok) throw new Error('Failed to download sorted list');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
  ...
  async function handleDownloadTopThreeStudents() {
    try {
      const res = await fetch('http://localhost:8080/api/auth/student-affairs/top-three-students-csv');
      if (!res.ok) throw new Error('Failed to download top three students list');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f5f5', position: 'relative' }}>
      ...
      <div style={{ padding: '2rem', textAlign: 'center', position: 'relative', transition: 'margin-left 0.3s', marginLeft: sidebarOpen ? 260 : 0 }}>
        ...
        {/* Faculty download rows */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, margin: '48px auto 0 auto', maxWidth: 900 }}>
          {facultyFiles.map((fac) => (
            <div key={fac.facultyId} style={{ display: 'flex', alignItems: 'center', width: 800, gap: 24, justifyContent: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', background: '#ececec', borderRadius: 12, padding: '12px 24px', fontSize: 20, width: 500, justifyContent: 'space-between', margin: '0 auto' }}>
                <span>Download the list of {fac.facultyName} (.csv)</span>
                <button
                  style={{
                    background: 'none',
                    border: 'none',
                    ...
                    padding: 0,
                    margin: 0,
                    cursor: fac.hasFile ? 'pointer' : 'not-allowed',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    pointerEvents: fac.hasFile ? 'auto' : 'none',
                    transition: 'filter 0.2s',
                  }}
                  disabled={!fac.hasFile}
                  onClick={fac.hasFile ? () => handleDownloadSortedList(fac.facultyId, fac.facultyName) : undefined}
                  title={fac.hasFile ? `Download ${fac.facultyName} List` : `${fac.facultyName} file not ready`}
                >
                  <img src={downloadIcon} alt="Download" style={{ width: 22, height: 22, filter: fac.hasFile ? 'none' : 'grayscale(1) opacity(0.6)' }} />
                </button>
              </div>
            </div>
          ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: 800, margin: '0 auto', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', width: 800, gap: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#ececec', borderRadius: 16, padding: '18px 24px', fontSize: 20, width: 500, justifyContent: 'space-between', marginLeft: 45 }}>
              ...
              <button style={{ background: 'none', border: 'none', outline: 'none', padding: 0, margin: 0, cursor: sortedDownloadActive ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: sortedDownloadActive ? 'auto' : 'none', transition: 'filter 0.2s' }} disabled={!sortedDownloadActive} onClick={sortedDownloadActive ? handleDownloadStudentAffairsSorted : undefined}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', width: 800, gap: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', background: '#ececec', borderRadius: 16, padding: '18px 24px', fontSize: 20, width: 500, justifyContent: 'space-between', marginLeft: 45 }}>
              ...
              <button style={{ background: 'none', border: 'none', outline: 'none', padding: 0, margin: 0, cursor: sortedDownloadActive ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: sortedDownloadActive ? 'auto' : 'none', transition: 'filter 0.2s' }} disabled={!sortedDownloadActive} onClick={sortedDownloadActive ? handleDownloadTopThreeStudents : undefined}>
                <img src={downloadIcon} alt="Download" style={{ width: 22, height: 22, filter: sortedDownloadActive ? 'none' : 'grayscale(1) opacity(0.6)' }} />
              </button>
            </div>
            <span style={{
